...
def print_betweenness_centrality(edgelist, v):
    ...
    # iterate through verticies for start node
    for vertex1 in vertices:
        tot = 0
        have_v = 0
        # for each start node, iterate through all verticies for the end node
        for vertex2 in vertices:

            # ensure that start and end aren't the same node and that neither of them are the vertex being calculated on
            if vertex1 != vertex2 and vertex1 != str(v) and vertex2 != str(v):

                # create a list of all shortest paths between v1 and v2
                paths = nx.all_shortest_paths(graph, vertex1, vertex2)

                # iterate through each path
                for path in paths:
                    # increment total for amount of paths
                    tot+=1
                    # check if target vertex is in path
                    if str(v) in path:
                        # increment to count how many have target vertex
                        have_v+=1
                        #print(path)
